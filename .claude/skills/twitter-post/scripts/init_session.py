"""
Twitter / X Session Initializer — Signal-file mode (no TTY needed)
One-time headed login to save a persistent Playwright browser session.

Usage:
    python init_session.py

A Chromium window opens. Log in manually, handle any "Unusual Activity"
check (enter your @handle when prompted), reach the X home feed, then:

    touch /tmp/x_session_done

Session is saved to .x_session/ and reused by post_twitter.py.
"""
...
SESSION_DIR = Path(os.getenv("TWITTER_SESSION_DIR", str(VAULT / ".x_session")))
SESSION_DIR.mkdir(parents=True, exist_ok=True)

SIGNAL_FILE = Path("/tmp/x_session_done")
SIGNAL_FILE.unlink(missing_ok=True)
TIMEOUT_SECONDS = 300

print("=" * 60)
print("  Twitter / X Session Initializer")
print("=" * 60)
print(f"  Session dir  : {SESSION_DIR}")
print(f"  Signal file  : {SIGNAL_FILE}")
print()
print("  A Chromium browser window will open.")
print("  → Log in to X (Twitter) manually")
print("  → If you see 'Enter phone or username' → type: ubaidraza8973")
print("  → Complete any 2FA / email verification")
print("  → Once you see the HOME FEED timeline, run:")
print()
print("       touch /tmp/x_session_done")
print()
